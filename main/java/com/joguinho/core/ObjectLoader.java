...
return loadModel(verticesArr, texCoordArr, normalArr, indicesArr);
    ...
    public Model loadModel(float[] vertices, float[] textureCoords, float[] normals, int[] indices) {
        ...
        unbind();
        return new Model(id, vertices.length / 3, indices.length);
    }
    ...
    public int loadTexture(String filename) throws Exception {
        ...
        try(MemoryStack stack = MemoryStack.stackPush()) {
            ...
            if(buffer == null)
                throw new Exception("Image file " + filename + "nao carregou " + STBImage.stbi_failure_reason());
    ...
    private void storeDataInAttribList(int atrribNo, int vertexCount, float[] data) {
        int vbo = GL15.glGenBuffers();
        vbos.add(vbo);
        GL15.glBindBuffer(GL15.GL_ARRAY_BUFFER, vbo);
        FloatBuffer buffer = Utils.storeDataInFloatBuffer(data);
        GL15.glBufferData(GL15.GL_ARRAY_BUFFER, buffer, GL15.GL_STATIC_DRAW);
        GL20.glVertexAttribPointer(atrribNo, vertexCount, GL11.GL_FLOAT, false, 0, 0);
